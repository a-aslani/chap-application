...
class DigitalProductsView extends StatefulWidget {
  ...
  DigitalProductsView({@required this.scaffoldKey});
...
class _DigitalProductsViewState extends State<DigitalProductsView> {
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<Map<String, dynamic>>(
      future: ApiService().fetchAllProducts('digital'),
      builder: (context, snapshot) {
        if (snapshot.hasError)
          AppSnackbar.show(widget.scaffoldKey, snapshot.error.toString(), 7);
        return snapshot.hasData
            ? ProductsList( response: snapshot.data, scaffoldKey: widget.scaffoldKey)
...
class ProductsListState extends State<ProductsList> {
  ...
  _getProducts() async {
    try {
      ...
      final response = await ApiService().fetchAllProducts('digital', page: _currentPage + 1);
